...
Gossiper.prototype.start = function(callback) {
  ...
  var finishup = function() {
    self.handleNewPeers(this.seeds);
    self.heartBeatTimer = setInterval(function() { self.my_state.beatHeart() }, 1000 );
    self.gossipTimer = setInterval(function() { self.gossip() }, 1000);
  }
  ...
  if(this.ip_to_bind) {
    if(this.port == null || this.port == 0) {
      ...
      var d = this.server.listen(0, this.ip_to_bind, function() {
        if(self.port == null) {
          self.port = d.address().port;
          self.peer_name    = [self.address, self.port.toString()].join(':');
